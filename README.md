...
Jarvis can act as the satellite-side client of the stock **Voice: Preview Edition** over the ESPHome Native API on TCP `6053`. Product mode is `Stock Voice PE / push-to-talk + continued conversation`: the centre button opens the first wake-free session and `continue_conversation` carries the follow-ups. The audio path is PCM16LE at 16 kHz mono in, and the same shape out in 512-sample chunks paced against the fixed 512 ms device ring buffer. Queue items are stamped `AudioFrame(source, generation, samples)` and the microphone end is an in-queue `EndOfStream` marker, so no buffered audio is lost; every milestone carries the `TurnContext` of its own turn; synthesis runs sentence by sentence on a dedicated executor off the event loop; `abort_run()` is the one generation-guarded close; media and `led_ring` state come from the device's own pushes. Hardware smoke test: `python scripts/_voice_pe_smoke.py [host]`, exit code = failed checkpoints.
