...
| `src/jarvis/everywhere/everywhere.spec.md` | Everywhere plane: immutable SelectionSnapshot, deterministic provider hierarchy, context transactions with revalidation, structured proofread, prompt library, named-pipe protocol to the native `Toastovac.Everywhere.Host.exe` | Fail closed per provider; STALE/CANCELLED never insert; no Electron, no second router, no cloud; password fields never read |
