...
| `whisper_backend` | `"auto"` | `"auto"`, `"mlx"`, `"faster-whisper"`, or `"openvino"`. `auto` never selects OpenVINO implicitly; OpenVINO is only used when chosen explicitly. |
| `whisper_model` | `"large-v3-turbo"` (NVIDIA) / `"medium"` | Canonical family. The shared OpenVINO catalog exposes `tiny`, `base`, `small`, `medium`, `large-v3`, `large-v3-turbo`; `"Large"` in the OpenVINO UI identifies `large-v3` explicitly. `.en`/distilled options belong to the other backends and are invalid for `cs`/`vi`/`cs+vi`. |
| `whisper_openvino_precision` | `"int8"` | Artifact selection: `"int8"` (NNCF INT8 weight-compressed IR, recommended) or `"fp16"`. Artifact precision only; runtime op precision is a separate compiler detail. Invalid values produce a visible `OV_MODEL_INCOMPLETE` error and the default. |
| `whisper_openvino_device` | `"NPU"` | Explicit inference target (`"NPU"`, `"CPU"`, `"GPU"`). No `AUTO`/`HETERO`, no hidden fallback: an unavailable selected device is the error `OV_NPU_UNAVAILABLE`. |
| `whisper_openvino_runtime_source` | `"installed"` | `"installed"` (registry `InstallDir` / setupvars layout) or `"wheel"` (exact owner-supplied cp313 wheels in a dedicated environment). One selected runtime source per worker. |
| `whisper_openvino_runtime_root` | `""` | Optional installed-root override; empty means installer discovery. An invalid explicit root is an actionable `OV_RUNTIME_NOT_FOUND` error, not a licence to pick another installation. |
| `whisper_openvino_python` | `""` | Optional CPython 3.13 x64 worker-interpreter override; empty means discover the managed compatible worker. The frozen desktop `sys.executable` is not the worker interpreter. |
...
### OpenVINO Adapter Contract

The `openvino` backend runs the IR Whisper pipeline in an isolated CPython 3.13 x64 worker (versioned stdio JSON protocol, request IDs, logs on stderr; no network server). The adapter preserves the same decode contract: transcription task, forced language per pass, outer VAD only, no cross-utterance conditioning, no-timestamp path, and the same `cs+vi` two-pass ranking with deterministic ties. Genuine statistics come from the matched GenAI companion (score semantics `ov-genai-whisper-logprob-v1`): `avg_logprob = sum_logprob / (token_count + 1)` over the returned hypothesis (beam search: cumulative recovered by reversing the length penalty), and `no_speech_prob` from the raw decoder distribution at the start-of-transcript position (full-vocabulary FP32 softmax before masking; continuation windows report `0.0`, matching the OpenAI reference). The adapter validates every row strictly — finite metrics, `0 ≤ no_speech_prob ≤ 1`, recognised semantics — and rejects missing/NaN/infinite/unscoped values with `OV_WHISPER_METRICS_UNAVAILABLE` before transcripts can trigger commands; the legacy permissive `None` path is not reachable for OpenVINO rows. Decoder state is reset between the two language passes and between utterances; a forced code echoed by the backend is not reported as independent detection. Companion provenance and the outstanding artifact state are recorded in `docs/openvino-npu-speech.md`.
