...
def load() -> bool:
    ...
    for cand in _dll_candidates():
        ...
        ABI_VERSION = abi
        # The DLL uses the process-wide COM apartment; initialize it here so
        # first-call enumeration/creation does not depend on some other
        # module's import order. S_FALSE (=already initialized) is fine.
        try:
...
def engine_telemetry(handle: Optional[int]) -> Optional[dict]:
    ...
    if int(_D.JarvisAeEngineReadTelemetry(ctypes.c_void_p(int(handle)), ctypes.byref(tel))) == 0:
        return {
...
def lane_telemetry(lane_handle: int) -> Optional[dict]:
    ...
    if int(_D.JarvisAeLaneReadTelemetry(
            ctypes.c_void_p(lane_handle), ctypes.byref(tel))) == 0:
