...
- **Field metrics + NPU banner.** Every input on the page (app preset, base URL, API key, chat / embedding / fast combos) gets `setMinimumHeight(36)` so the shared QSS padding cannot squash it and clip the label or placeholder (matches the `ModelsPage` dropdowns). When the native OpenVINO NPU service answers on `:8010` the page probes it **once** and adds **one** banner naming the pre-filled embedding model, base URL and dimensions; that pre-filled value survives `_fill_combo`, which preserves an existing combo text.
...
**WhisperSetupPage** — Always shown, right after Provider Choice (it has no LLM dependencies and its model selection informs the VRAM budget on the Models page). Backend selector (`auto` / faster-whisper / MLX / OpenVINO / Intel NPU); the OpenVINO page adds artifact precision (**INT8 recommended** / FP16), runtime source (installed tree / owner wheel), optional runtime-root and worker-interpreter overrides, and five separate status rows (runtime discovery + actual Core build, GenAI/tokenizer companion, NPU device, selected model cache, pipeline). Preflight is lightweight: registry `InstallDir` + file validation on the UI thread, the handshake (Core version, companion, device enumeration) in a short-lived worker subprocess; the model download runs off the Qt thread with per-asset progress and is resumable. The OpenVINO catalog is multilingual-only and always lists `large-v3` and `large-v3-turbo`; the faster-whisper turbo filter applies only to that backend. Language mode toggle (multilingual vs English-only), then model size selection via a slider. Apple Silicon: additional FFmpeg and MLX Whisper installation buttons. `validatePage` returns the save result: a failed write keeps the user on the page with a visible error; a deliberate "configure later" leaves the status rows pending without claiming readiness. Saves the complete backend/model/precision/language/runtime selection through the existing config mechanism (only non-default keys, unrelated keys preserved, 0o600 kept) and reconstructs it on reopen. Exposes a `get_whisper_vram_mb()` static method used by `ModelsPage` for accurate total VRAM calculation. `nextId` routes to the Welcome/Status page (Ollama) or the OpenAI-compatible config page, based on the provider choice made earlier.
